...
namespace DrivingSclApp.Areas.Indexes.Controllers
{
    ...
    public class zDocTypeController : Controller
    {
        ...
        {
            var Data = db.ZDOCTYPE.OrderBy(x => x.NB).ToList();
            return Json(Data.ToDataSourceResult(request), JsonRequestBehavior.AllowGet);
        }
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Create([DataSourceRequest] DataSourceRequest request, ZDOCTYPE model)
        {
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Update([DataSourceRequest] DataSourceRequest request, ZDOCTYPE model)
        {
            using (var transaction = db.Database.BeginTransaction())
            {
                if (ModelState.IsValid)
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult Destroy([DataSourceRequest] DataSourceRequest request, ZDOCTYPE model)
        {
            using (var transaction = db.Database.BeginTransaction())
            {
                try
        ...
        {
            string ss = MyDataBase.RunQuery("select count(*) from ZDOCTYPE where DOCTYPE='" + param1 + "'");
